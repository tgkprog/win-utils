...
Function GetVolume(VolumeLevel As Long) As Long
...
'mixerGetControlDetails Lib "winmm.dll" Alias "mixerGetControlDetailsA"
'(ByVal hmxobj As Long, pmxcd As MIXERCONTROLDETAILS, ByVal fdwDetails As Long) As Long
'uUnsigned.dwValue
    'RetValue = mixerGetControlDetails(hmx, uDetails, MIXER_SETCONTROLDETAILSF_VALUE)
    RetValue = mixerGetControlDetails(hmx, uDetails, MIXER_SETCONTROLDETAILSF_VALUE)
    uDetails.item = 0
    uDetails.dwControlID = uMixerControl.dwControlID
    uDetails.cbStruct = Len(uDetails)
    uDetails.cbDetails = Len(uUnsigned)
    GetVolume = uDetails.cbDetails * 20
 Exit Function
